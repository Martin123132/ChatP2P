...
from .gremlinchat.approvals import (
    approval_for_task,
...
from .gremlinchat.config import (
    ApprovedRepo,
    RunbookPolicy,
    default_home,
    ensure_home,
...
from .gremlinchat.pairing import create_invite_code, parse_invite_code
from .gremlinchat.relay import RelayClient, create_relay_http_server
from .gremlinchat.reports import write_task_report
from .gremlinchat.runbooks import check_runbook_approval, execute_runbook, runbook_catalog, runbook_result_json
from .gremlinchat.security import EncryptedEnvelope, ReplayGuard, derive_room_key, open_message, safety_phrase, seal_message
from .gremlinchat.tasks import create_pair_hello, create_task_request, create_task_result, verify_pair_hello
        ...
        json.dumps(
            {
                ...
                "x25519_public_key": x25519_identity.public_key,
...
def create_room(args: argparse.Namespace) -> None:
    ...
    room_response = client.create_room(ttl_seconds=args.ttl_seconds)
    ...
    parsed = parse_invite_code(invite)
    save_room(
        {
            "room_id": parsed.room_id,
            "relay_url": parsed.relay_url,
            "relay_token": parsed.relay_token,
            "pair_secret": parsed.pair_secret,
        ...
        json.dumps(
            {
                ...
                "expires_at": parsed.expires_at,
                "invite_code": invite,
                "note": "Share this code privately. It expires and the relay room locks after two identities join.",
            },
            indent=2,
            sort_keys=True,
        )
        ...
        json.dumps(
            {
                ...
                "next_step": "Both people should confirm the same safety phrase before enabling write runbooks.",
            },
            indent=2,
            sort_keys=True,
        )
    ...
    for record in messages:
        ...
        if envelope.get("protocol") == "gremlinchat.pair-hello.v1":
            if envelope.get("sender_node_id") != identity.node_id and verify_pair_hello(envelope):
                ...
                room["safety_phrase"] = safety_phrase(
                    room["pair_secret"],
                    [identity.public_key, envelope["sender_public_key"]],
                )
                updated = True
            continue
        ...
        try:
            message = open_message(
                ...
                room_key=_room_key(room, identity, x25519_identity),
        ...
        json.dumps(
            {
                ...
                "peer_node_id": room.get("peer_node_id"),
...
def request_runbook(args: argparse.Namespace) -> None:
    ...
    request = create_task_request(runbook=args.runbook, payload=payload)
    ...
    response = RelayClient(room["relay_url"]).post_envelope(
        room_id=room["room_id"],
...
def process_room(args: argparse.Namespace) -> None:
    summary = _process_room_once(_home(args.home), args.room_id)
...
def _process_room_once(home: Path, room_id: str | None) -> dict:
    ...
    for record in _fetch_room_messages(room):
        ...
        if envelope_data.get("protocol") != "gremlinchat.envelope.v1":
            continue
        if envelope_data.get("sender_node_id") == identity.node_id:
            continue
        ...
        if existing_approval and existing_approval.get("status") == "approved":
            ...
            reply_envelope = seal_message(
                room_id=room["room_id"],
                sender=identity,
                room_key=_room_key(room, identity, x25519_identity),
                message=reply,
            )
            relay_response = RelayClient(room["relay_url"]).post_envelope(
                room_id=room["room_id"],
                relay_token=room["relay_token"],
            ...
            replies.append({"task_id": message["task_id"], "runbook": runbook, "relay_response": relay_response})
            continue
        else:
            approval_check = check_runbook_approval(
                runbook,
                payload,
                policy=policy,
        ...
        if existing_approval and approval_override:
            mark_approval_consumed(home, str(existing_approval["approval_id"]))
        report_paths = write_task_report(
            home,
            {
                "direction": "outgoing",
                "task_id": message["task_id"],
...
def _owner_rejected_result(runbook: str) -> dict:
    ...
    return {
        ...
        "runbook": runbook,
        "status": "rejected",
        "summary": "Local owner rejected this GremlinChat request.",
        "output": {"error": "owner_rejected"},
        ...
        json.dumps(
            {
                "home": str(home),
                "node_id": identity.node_id,
                "policy": runbook_catalog(load_policy(home)),
            },
            indent=2,
            sort_keys=True,
        )
...
def approve_repo(args: argparse.Namespace) -> None:
    ...
    repo = ApprovedRepo(
        ...
        allow_pull_ff_only=args.allow_pull,
        allow_tests=list(args.allow_test),
    )
...
def _load_room(home: Path, room_id: str | None) -> dict:
    from .gremlinchat.config import load_rooms
...
def _fetch_room_messages(room: dict) -> list[dict]:
    response = RelayClient(room["relay_url"]).messages_after(
        room_id=room["room_id"],
        relay_token=room["relay_token"],
        after=-1,
    )
...
def _room_key(room: dict, identity: NodeIdentity, x25519_identity) -> bytes:
    ...
    return derive_room_key(
        local_private_key=x25519_identity.private_key,
        peer_public_key=peer_x25519,
        pair_secret=room["pair_secret"],
        participant_public_keys=[identity.public_key, peer_public],
    )


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="gremlinchat", description="GremlinChat private control room")
    parser.add_argument("--home", default=None, help="GremlinChat config directory. Defaults to LOCALAPPDATA/GremlinChat.")
    subcommands = parser.add_subparsers(dest="command", required=True)

    setup_parser = subcommands.add_parser("setup", help="Create local identity and default policy")
    setup_parser.set_defaults(func=setup)

    status_parser = subcommands.add_parser("status", help="Show local GremlinChat status")
    status_parser.set_defaults(func=show_status)

    stop_parser = subcommands.add_parser("emergency-stop", help="Disable all remote runbook requests")
    stop_parser.set_defaults(func=emergency_stop)
    ...
    room_request = room_subcommands.add_parser("request", help="Send an encrypted runbook request to the paired machine")
    ...
    room_process = room_subcommands.add_parser("process", help="Process encrypted runbook requests for this machine")
    ...
    runbook_list = runbook_subcommands.add_parser("list", help="List runbooks and policy")
    runbook_list.set_defaults(func=list_runbooks)
    ...
    approve = runbook_subcommands.add_parser("approve-repo", help="Approve one repo path for repo runbooks")
    ...
    approval_list = approval_subcommands.add_parser("list", help="List pending and historical approvals")
    approval_list.set_defaults(func=list_approvals_command)
    approval_approve = approval_subcommands.add_parser("approve", help="Approve a pending request")
    approval_approve.add_argument("approval_id")
    approval_approve.set_defaults(func=decide_approval_command)
    approval_reject = approval_subcommands.add_parser("reject", help="Reject a pending request")
