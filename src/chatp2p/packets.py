...
class JobLeaseRenewal:
    ...
    def create(
        ...
        *,
        ...
        unsigned = {
            "packet_type": "job.lease-renewal.v1",
            "renewal_id": f"lease_renewal_{uuid.uuid4().hex}",
            "lease_id": lease_id,
            "job_id": job_id,
            "node_id": node.node_id,
            "node_public_key": node.public_key,
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "JobLeaseRenewal":
        return cls(**data)
    ...
    def verify_signature(self) -> bool:
        if self.packet_type != "job.lease-renewal.v1":
            return False
        node = NodeIdentity(node_id=self.node_id, public_key=self.node_public_key)
        return node.verify(canonical_bytes(self.unsigned_dict()), self.node_signature)
