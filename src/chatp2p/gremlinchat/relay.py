"""Minimal GremlinChat relay.

The relay is deliberately dumb: it stores room metadata and opaque encrypted
envelopes. It never sees decrypted task contents, logs, repo paths, or machine
state.
"""
...
class GremlinRelay:
    ...
    def create_room(self, *, ttl_seconds: int = 600) -> RelayRoom:
        room = RelayRoom(
            room_id=f"room_{secrets.token_urlsafe(18)}",
...
def create_relay_http_server(relay: GremlinRelay | None = None, host: str = "127.0.0.1", port: int = 8778):
    ...
    class RelayHandler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:
            ...
            try:
                if parsed.path == "/v1/rooms":
                    ...
                    with lock:
                        room = relay.create_room(ttl_seconds=ttl_seconds)
                    _json_response(
                        self,
                        201,
                        {
                            "room_id": room.room_id,
                            "relay_token": room.token,
                ...
                if len(parts) == 4 and parts[:2] == ["v1", "rooms"] and parts[3] == "messages":
                    request = _read_json(self)
                    token = str(request.get("relay_token", ""))
                    envelope = request["envelope"]
                    with lock:
                        response = relay.append_envelope(parts[2], token, envelope)
        ...
        def do_GET(self) -> None:
            ...
            if len(parts) == 4 and parts[:2] == ["v1", "rooms"] and parts[3] == "messages":
                ...
                try:
                    with lock:
                        response = relay.messages_after(parts[2], token, after)
...
class RelayClient:
    ...
    def post_envelope(self, *, room_id: str, relay_token: str, envelope: dict[str, Any]) -> dict[str, Any]:
        return self._request(
            ...
            {"relay_token": relay_token, "envelope": envelope},
        )
    ...
    def messages_after(self, *, room_id: str, relay_token: str, after: int = -1) -> dict[str, Any]:
        ...
        return self._request(
            "GET",
            f"/v1/rooms/{room_id}/messages?token={quote(relay_token)}&after={after}",
        )
