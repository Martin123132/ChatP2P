"""Strict GremlinChat v0.1 runbook executor."""
...
from .config import ApprovedRepo, RunbookPolicy
    ...
    *,
    ...
    try:
        ...
        if runbook == "tests.run_allowlisted":
            ...
            if (
                runbook in policy.enabled_write_runbooks
                and repo is not None
                and test_name in repo.allow_tests
            ):
    ...
    *,
    ...
    try:
        ...
        if runbook == "presence.ping":
            output = {"pong": True, "server_time": round(time.time(), 3)}
            return _result(True, runbook, "completed", "Presence ping completed.", output, started_at, home)
        if runbook == "machine.status":
            output = _machine_status()
            return _result(True, runbook, "completed", "Machine status collected.", output, started_at, home)
        if runbook == "repo.status":
            repo = _approved_repo(payload, policy)
            output = _repo_status(Path(repo.path))
            return _result(True, runbook, "completed", f"Repo status collected for {repo.name}.", output, started_at, home)
        ...
        if runbook == "chatp2p.doctor":
            output = run_node_doctor(
                NodeDoctorConfig(
                    home=Path(payload.get("mesh_home", ".mesh")),
                    model=payload.get("model"),
                    coordinator_url=payload.get("coordinator_url"),
                    timeout_seconds=float(payload.get("timeout_seconds", 2.0)),
                )
            )
            return _result(True, runbook, "completed", "ChatP2P doctor completed.", output, started_at, home)
        if runbook == "repo.pull_ff_only":
            repo = _approved_repo(payload, policy)
            ...
            output = _repo_pull_ff_only(Path(repo.path))
            return _result(True, runbook, "completed", f"Fast-forward pull completed for {repo.name}.", output, started_at, home)
        if runbook == "worker.restart_named":
            name = str(payload.get("worker_name", ""))
            command = policy.managed_workers.get(name)
            if not command:
                raise PermissionError(f"Worker {name!r} is not managed by GremlinChat.")
            output = _run_command(command, cwd=None, timeout_seconds=30)
            return _result(True, runbook, "completed", f"Restart command ran for {name}.", output, started_at, home)
        if runbook == "tests.run_allowlisted":
            repo = _approved_repo(payload, policy)
            test_name = str(payload.get("test_name", ""))
            ...
            output = _run_command(command, cwd=Path(repo.path), timeout_seconds=120)
            return _result(True, runbook, "completed", f"Allowlisted test ran: {test_name}.", output, started_at, home)
...
def _approved_repo(payload: dict[str, Any], policy: RunbookPolicy) -> ApprovedRepo:
    ...
    for repo in policy.approved_repos:
        ...
        if repo_name and repo.name == repo_name:
            ...
            return repo
        if requested_path:
            requested = _resolved_path(str(requested_path))
            if requested == base:
                return repo
