...
GremlinChat is the product layer on top of the signed ChatP2P core: a private control room for two humans and their local machines. It uses one-time invite codes, signed node identities, X25519 room keys, AES-GCM encrypted envelopes, a relay that only sees opaque ciphertext, and a strict runbook policy so a partner cannot send arbitrary shell commands.
...
Runbooks are deliberately narrow in v0.1:
...
- `worker.status`
- `chatp2p.doctor`
- owner-enabled `repo.pull_ff_only`
- owner-enabled `worker.restart_named`
- owner-enabled `tests.run_allowlisted`

Use `gremlinchat emergency-stop` to immediately disable remote runbook requests.

Write-capable requests that are not pre-approved are queued for the local owner:
...
Task results are written as Codex-friendly JSON and Markdown reports under the local GremlinChat home:
